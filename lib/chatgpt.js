...
const fillPromptTextarea = async (page, ai_prompt) => {
  ...
  await page.$eval(
    ...
    (element, my_prompt) => {
      ...
      const inserted = document.execCommand("insertText", false, my_prompt);
      if (!inserted) {
...
const _runPrompt = async (page, ai_prompt, timeout = 30000, attempt = 0) => {
  ...
  try {
    ...
    return _runPrompt(page, ai_prompt, timeout, attempt + 1);
  }
